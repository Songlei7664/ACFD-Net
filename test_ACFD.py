...
import utils.utils_logging as logging
import utils.metrics as metrics
from models.ACFD import ACFDnet
...
def main():
    ...
    print("\n1. Define Model")
    model = ACFDnet(max_depth=args.max_depth).to(device)
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        if args.save_visualize:
            save_path = os.path.join(result_path, filename[0])
            print("Saving result in to %s" % save_path)

            pred_depth_scaled = (pred_d * 1000.0).squeeze().cpu().numpy()
            pred_depth_scaled = pred_depth_scaled.astype(np.uint16)
            pred_depth_scaled -= pred_depth_scaled.min()
            uint16_img = pred_depth_scaled / (pred_depth_scaled.max() - pred_depth_scaled.min())
            uint16_img *= 255
